...
impl<T> Node<T> {
    ...
    {
        let node = Node {
            next: PauseMutex::from(None),
            prev: PauseMutex::from(None),
            value,
            _pin: PhantomPinned,
        };
...
impl<T> LinkedList<T> {
    ...
    /// Internal function used to link a node to the next one.
    #[inline(always)]
    fn link_next(
        &self,
        token: PauseToken,
        ...
        if let Some(prev) = prev {
            ...
            self.tail.set(token, next);
        }
    }

    /// Internal function used to link a node to the previous one.
    #[inline(always)]
    fn link_prev(
        &self,
        token: PauseToken,
    ...
    pub unsafe fn remove(&self, token: PauseToken, node: Pin<&Node<T>>) {
        ...
        self.link_next(token, prev, next);
    ...
    pub unsafe fn insert_after(
        ...
        let old_next = if let Some(prev) = prev {
            prev.next.replace(token, Some(node.into()))
        } else {
            self.head.replace(token, Some(node.into()))
        };

        let old_next = old_next.map(|ptr| unsafe { ptr.get_ref() });

        self.link_prev(token, old_next, Some(node));
    }
    ...
    pub unsafe fn insert_before(
        ...
        let old_prev = if let Some(next) = next {
            next.prev.replace(token, Some(node.into()))
        } else {
            self.tail.replace(token, Some(node.into()))
        };

        let old_prev = old_prev.map(|ptr| unsafe { ptr.get_ref() });

        self.link_next(token, old_prev, Some(node));
    }
